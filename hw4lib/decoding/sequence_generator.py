...
class SequenceGenerator:
    ...
    def _apply_repeat_penalty(
        self, logits: torch.Tensor, sequences: torch.Tensor, penalty: float = 1.0
    ) -> torch.Tensor:
        """
            ...
            for batch_idx in range(sequences.size(0)):
                for beam_idx in range(sequences.size(1)):
                    ...
                    logits[batch_idx, beam_idx, unique_tokens] = logits[
                        batch_idx, beam_idx, unique_tokens
                    ] / torch.where(
                        logits[batch_idx, beam_idx, unique_tokens] > 0,
                        torch.full_like(
                            logits[batch_idx, beam_idx, unique_tokens], penalty
                        ),
                        torch.full_like(
                            logits[batch_idx, beam_idx, unique_tokens], 1.0 / penalty
                        ),
                    )
    ...
    def generate_greedy(
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            # Get the score f the chosen token
            ...
            # Add the score of the current token to the cumulative score
            scores = torch.where(finished, scores, scores + token_scores)

            # Append the chosen token to the sequence
            x = torch.cat([x, next_tokens.unsqueeze(1)], dim=1)

            # Check if the newly added token is the EOS token
            is_eos = next_tokens == self.tokenizer.eos_id

            # Update the finished flag for sequences that just generated EOS
    ...
    def generate_beam(
        ...
        while beams.size(2) < max_len:
            B, n_beams, L = beams.shape
            next_scores = torch.zeros(B, n_beams, V, device=device)

            for w in range(n_beams):
                
                batch_seqs = beams[:, w, :]     
                out = self.score_fn(batch_seqs)  
                ...
                if out.dim() == 3:
                    ...
                    raise ValueError(f"Unexpected score_fn output dim {out.dim()}")

                if temperature != 1.0:
                    logits = logits / temperature

                # repeat penalty per batch item
                if repeat_penalty != 1.0:
                    for b in range(B):
                        seen = set(batch_seqs[b].tolist())
                        for t in seen:
                            logits[b, t] /= repeat_penalty

                next_scores[:, w, :] = logits  

            total_scores = scores.unsqueeze(-1) + next_scores 

            flat_scores = total_scores.view(B, n_beams * V)  
            top_scores, top_idx = torch.topk(flat_scores, beam_width, dim=-1) 

            prev_beam = top_idx // V    
            new_token = top_idx % V       
    
            beams = torch.gather(beams, 1,prev_beam.unsqueeze(-1).expand(-1, -1, L)) 
            beams = torch.cat([beams, new_token.unsqueeze(-1)], dim=-1) 
    ...
    def generate_sample(
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            filtered_logits = self._filter_logits(
                next_scores, temperature, top_k, top_p
            )
            ...
            token_scores = log_probs.gather(1, next_tokens.unsqueeze(1)).squeeze(
                1
            )  # (batch_size,)
    ...
    def post_process_sequence(
        seq: torch.Tensor, tokenizer: H4Tokenizer
    ) -> torch.Tensor:
        """
