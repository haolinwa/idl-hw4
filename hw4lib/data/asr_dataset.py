...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            feat = np.load(self.fbank_files[i])  # numpy, (F, T)
            ...
            if self.partition != "test-clean":
                ...
                arr = np.load(self.text_files[i], allow_pickle=True)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [feat.transpose(0, 1) for feat, _, _ in batch]  # each (T, F)
        ...
        # (for acoustic features, padding with 0.0 is standard)
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [shifted for _, shifted, _ in batch]  # list of (T,)
            batch_golden       = [golden  for _, _, golden in batch]   # list of (T,)
