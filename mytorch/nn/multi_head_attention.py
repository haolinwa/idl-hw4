...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        if key_padding_mask is not None:
            key_mask = key_padding_mask.astype(bool).reshape(self.N, 1, 1, self.S)
        ...
        if attn_mask is not None:
            attention_mask = attn_mask.astype(bool).reshape(1, 1, self.L, self.S)
            attention_mask = np.broadcast_to(attention_mask, (self.N, self.num_heads, self.L, self.S))
            mask = attention_mask if mask is None else np.logical_or(mask, attention_mask)
